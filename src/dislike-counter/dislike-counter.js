...
const DISLIKE_TEXT_MARKER = "ytt-dislike-text";

function buttonRoot(button) {
    return button ? (button.shadowRoot || button) : null;
}

function findButtonTextContainer(button) {
    const root = buttonRoot(button);
    if (!root) return null;
    return root.querySelector(".yt-spec-button-shape-next__button-text-content")
        || root.querySelector(".ytSpecButtonShapeNextButtonTextContent");
}

/**
 * YouTube marks an unused button text slot with `is-empty` and styles that
 * attribute to collapse the slot. Clearing it is what hands spacing back to
 * YouTube's own icon+text rules; forcing `display` instead leaves the
 * `is-empty` spacing rules applying and the count lands in the wrong place.
 */
function activateTextSlot(container) {
    if (!container) return null;
    container.removeAttribute("is-empty");
    // YouTube renders the visible string in an inner span[role="text"]; write
    // there when it exists so the count is styled the way the like count is.
    const inner = container.matches('span[role="text"]')
        ? container
        : container.querySelector('span[role="text"]');
    if (inner) inner.removeAttribute("is-empty");
    return inner || container;
}

/**
 * Get a text slot on the dislike button that YouTube will lay out exactly as it
 * lays out the like count.
 *
 * Preferred path: the dislike button's own slot, with `is-empty` cleared.
 * Fallback: clone the like button's slot wholesale. A clone carries the same
 * classes and inner structure, so the dislike button becomes structurally
 * identical to the like button and native CSS positions it with no help from
 * us. This is how Return YouTube Dislike does it, and it is why no padding or
 * margin arithmetic is needed anywhere in this file.
 */
function ensureDislikeTextSlot(dislikeBtn, likeBtn) {
    const root = buttonRoot(dislikeBtn);
    if (!root) return null;

    const existingClone = root.querySelector("." + DISLIKE_TEXT_MARKER);
    if (existingClone) return activateTextSlot(existingClone);

    const own = findButtonTextContainer(dislikeBtn);
    if (own) return activateTextSlot(own);

    const template = findButtonTextContainer(likeBtn);
    if (!template) return null;

    const clone = template.cloneNode(true);
    clone.classList.add(DISLIKE_TEXT_MARKER);
    const slot = activateTextSlot(clone);
    if (slot) slot.textContent = "";
    dislikeBtn.appendChild(clone);
    return slot;
}

function resetDislikeTextSlot(container) {
    if (!container) return;
    if (container.classList && container.classList.contains(DISLIKE_TEXT_MARKER)) {
        container.remove();
        return;
    }
    const slot = container.matches('span[role="text"]')
        ? container
        : container.querySelector('span[role="text"]') || container;
    slot.textContent = "";
    slot.removeAttribute("data-ytt-dislike");
    container.setAttribute("is-empty", "");
    container.style.removeProperty("display");
    container.removeAttribute("data-ytt-dislike");
}
...
let lastRenderedDislikeBtn = null;

/**
 * Answer "is there anything to do?" without touching the DOM broadly. Holding on
 * to the button we last rendered lets the common case -- YouTube mutated
 * something unrelated -- cost a couple of property reads instead of eight
 * full-document traversals.
 */
function isDislikeCountUpToDate(videoId) {
    const btn = lastRenderedDislikeBtn;
    if (!btn || !btn.isConnected) return false;
    if (btn.dataset.yttDislikeVideo !== videoId) return false;
    const root = btn.shadowRoot || btn;
    return !!(root.querySelector("[data-ytt-dislike]") || root.querySelector(".ytt-hijacked-label"));
}
...
function tryRenderDislike() {
  ...
  // Cheap check first. findDislikeButton/findLikeButton each walk the whole
  // document and every shadow root once per selector, which is far too
  // expensive to run on every DOM mutation YouTube makes -- doing it before
  // this guard is what pins the main thread and freezes the page.
  if (isDislikeCountUpToDate(videoId)) return;

  const dislikeBtn = findDislikeButton();
  const likeBtn = findLikeButton();
  if (!dislikeBtn) return;
  lastRenderedDislikeBtn = dislikeBtn;
...
function doRender(data, videoId, btn) {
    ...
    // === Strategy: hand the count to YouTube's own icon+text layout ===
    // Rather than positioning the number ourselves, give the dislike button a
    // text slot in the same state YouTube keeps the like button's in, then flip
    // the button from icon-only to icon+text. YouTube's CSS then spaces both
    // halves of the segmented pill identically, with no overrides from us.
    const textSlot = !isShorts ? ensureDislikeTextSlot(btn, findLikeButton()) : null;

    if (textSlot) {
        textSlot.textContent = countText;

        const container = (textSlot.closest && textSlot.closest(".yt-spec-button-shape-next__button-text-content"))
            || textSlot.parentElement
            || textSlot;
        // Mark the slot itself as well as its container: we write into the inner
        // span, and an unmarked target makes the observer treat our own write as
        // a YouTube change and run another full pass.
        if (container && container.setAttribute) {
            container.setAttribute("data-ytt-dislike", "true");
        }
        if (textSlot.setAttribute) {
            textSlot.setAttribute("data-ytt-dislike", "true");
        }
        ...
        if (btn.classList.contains("ytSpecButtonShapeNextIconButton")) {
            btn.classList.remove("ytSpecButtonShapeNextIconButton");
            btn.classList.add("ytSpecButtonShapeNextIconLeading");
            btn.setAttribute("data-ytt-class-swapped", "true");
        }
...
/**
 * YouTube mutates the watch page continuously, and each burst arrives as many
 * separate records. Coalesce them into one pass per frame so a busy page cannot
 * queue up an unbounded amount of work on the main thread.
 */
function scheduleRenderDislike() {
    ...
    globalObserver = new MutationObserver((mutations) => {
        const isSelfMutation = mutations.every(m => {
            ...
            return target.nodeType === 1 && (
                ...
                target.closest?.("[data-ytt-dislike]")
            );
        });

        if (!isSelfMutation) {
            scheduleRenderDislike();
        }
    });
...
function removeDislikeCount() {
    ...
    // Remove cloned slots and put YouTube's own ones back into their empty state
    document.querySelectorAll("." + DISLIKE_TEXT_MARKER).forEach(el => el.remove());
    document.querySelectorAll("[data-ytt-dislike]").forEach(resetDislikeTextSlot);
    ...
    document.querySelectorAll("[data-ytt-class-swapped]").forEach(btn => {
        ...
        btn.classList.remove("ytSpecButtonShapeNextIconLeading");
        btn.removeAttribute("data-ytt-class-swapped");
        delete btn.dataset.yttDislikeVideo;
    });

    lastRenderedDislikeBtn = null;
